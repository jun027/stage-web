...
import {
  CATEGORY_TYPE as LOTTERY_CATEGORY_TYPE,
  useLotteryData,
} from '@/sections/common/config/lottery-config'
import { Skeleton, Stack } from '@mui/material'
import { useTranslations } from 'next-intl'
import Image from 'next/image'

export default function SportView() {
  const t = useTranslations('Category.Lottery')
  const { data } = useLotteryData()
  const { isLoading, list } = useCategoryGetGame(LOTTERY_CATEGORY_TYPE, data)
        ...
        <div className="flex flex-col lg:flex-row items-center gap-x-28">
          <div className="aspect-[620/720] w-full lg:w-[620px]">
            <Image
              ...
              src="/images/cover-lottery-01.png"
              alt="真人"
              width={620}
              height={720}
            />
          ...
          <div className="flex flex-col items-center lg:items-start gap-y-8 max-w-[440px]">
            <div className="flex flex-col gap-y-2">
              <h1 className="text-4xl font-bold text-[#A2D834]">{t('Title')}</h1>
